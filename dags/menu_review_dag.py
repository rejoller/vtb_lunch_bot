    ...
    def get_openai_text(response_tesseract):
        ...
        gpt_response = client.responses.create(
            ...
            2. Итак сегодня 2 декабря у нас на гарнир <b>гречка</b> и <b>пюре</b>. Салаты и супы тоже вполне солидные
                ...
                , напиши меню на основе этого распознанного изображения и указывай текущую дату в формате [16 Декабря, Вторник] (в тексте могут быть неточности так как изображение прогонялось через библиотеку tesseracrt, учитывай это и исправляй артефакты если заметишь)
            ...
            Используй только такие HTML теги: <b>, <i>     Не используй никакие другие теги.
                Не используй символы вроде «умных» кавычек. Используй только обычные: " и ', ты работаешь для сотрудников банка ВТБ, так как столовая находится там : {response_tesseract}"""
        )
